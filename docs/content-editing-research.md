...
Reuse its [packet contract](../schemas/object-dossier-packet.schema.json), [validation](../scripts/lib/object-dossier/validate.ts), [identity resolution](../scripts/lib/object-dossier/resolve.ts), [import planning and transactional writes](../scripts/lib/object-dossier/import.ts), and ingestion bookkeeping. The CLI already calls these modules, so a future authenticated server endpoint can reuse them. Keep validation and authorisation on the server and map errors to understandable form fields. Import logs provide useful execution history, but do not constitute contributor review or publication approval.
