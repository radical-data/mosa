...
One event is one file in `apps/website/src/content/events/`. That file holds the event's facts and both languages together. A glob-based loader discovers the files and validates them against the shared event schema when the site builds. Adding, changing or withdrawing an event is a single file operation; no template, index or constant has to be edited alongside it.

Name the file `YYYY-MM-DD-short-slug.json`. The loader derives the entry ID from the filename; the date inside the file is what the site reads.
...
A date-only event is displayed as a calendar date. Include its `timeZone` when known: this determines when the local day ends without inventing an hour. A timed event is displayed with its hour, rendered in its own `timeZone` and in the reader's language. Impossible dates, invalid instants and unrecognised time zones fail validation.

## Upcoming and past

The site splits events itself. An event stays in the programme until the end of its calendar day in `timeZone`, then appears under past events, ordered most recent first. For timed events, the instant is converted to that zone to determine its local date. Date-only events without a zone use UTC; add the zone when known to get the correct local boundary.

Upcoming events are ordered soonest first, and the first one is shown expanded. For ordering only, a date with no hour uses noon UTC as a stable reference. It is still displayed without an hour.
...
`apps/website/src/data/load-events.ts` loads every event file and applies the shared schema from `src/data/event-schema.ts`. An invalid file fails the build with its filename and validation errors. It checks the date rules above, that both languages are present with matching optional fields, that an image and its description travel together, and that copy uses only the same trusted inline markup allowed in page copy (`apps/website/src/i18n/markup.ts`). Unknown fields are rejected, so a stray key is never silently ignored.

`apps/website/src/data/events.test.ts` tests the schema and the ordinary date/display functions with fixtures. It also validates the current event files without requiring particular titles, IDs or counts, so adding or deleting an event does not require changing tests.
